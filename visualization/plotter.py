...
class RadarPlotter:
    ...
    def _plot_results(self, results: RadarResults):
        ...
        # Diagramas IQ
        self._plot_iq_diagram(results, row=2, chirp='up')
        self._plot_iq_diagram(results, row=2, chirp='down', col=1)
        
        # FFT
        self._plot_fft(results, chirp='up', subplot_idx=2)
    ...
    def _plot_fft(self, results, chirp, subplot_idx):
        ...
        freqs = np.fft.fftshift(np.fft.fftfreq(len(signal), 1/self.config.Fs))
